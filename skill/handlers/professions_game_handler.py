...
from skill.texts import PROFESSIONS_GAME_END
...
class ProfessionsGameHandler(Handler):
    ...
    async def go_next_handle(self, alice_request):
        ...
        if questions is None:
            ...
            await self.dispatcher.storage.update_data(
                user_id, professions_game_questions=questions
            )
        ...
        try:
            ...
            await self.dispatcher.storage.set_state(
                user_id, ProfessionsGameStates.PROFESSIONS_GAME_END
            )
        ...
        suggests = ["Не знаю"]
        await self.dispatcher.storage.set_state(
            user_id, ProfessionsGameStates.PROFESSIONS_CHECK_ANSWER
        )
        return alice_request.response(f"Кто {question['question']}?", buttons=suggests)
    ...
    def register_handlers(self):
        ...
        self.dispatcher.register_request_handler(
            self.go_next_handle, state=ProfessionsGameStates.PROFESSIONS_GO_NEXT
        )
