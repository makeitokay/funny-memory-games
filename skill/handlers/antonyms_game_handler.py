...
from skill.texts import (ANTONYM_DONT_KNOW, ANTONYM_NEXT_WORD,
                         ANTONYM_WRONG_ANSWER, ANTONYMS_GAME_END,
                         ANTONYMS_RIGHT_ANSWER, ANTONYMS_TIP_ALREADY_USED)
from skill.texts import get_dynamic_text as d
from skill.utils.antonyms_game_utils import (get_random_antonyms,
...
class AntonymsGameHandler(Handler):
    ...
    async def game_process_handle(self, alice_request):
        ...
        if questions is None:
            ...
            await self.dispatcher.storage.update_data(
                user_id, antonyms_game_questions=questions
            )
        ...
        if questions.current is None or answer == questions.current[1]:
            ...
            if question is None:
                await self.dispatcher.storage.set_state(
                    user_id, AntonymsGameStates.ANTONYMS_GAME_END
                )
                suggests = ["Хочу отгадывать антонимы!", "Главное меню"]
                return alice_request.response(
                    ANTONYMS_GAME_END.text, buttons=suggests, tts=ANTONYMS_GAME_END.tts
                )
            await self.dispatcher.storage.update_data(
                user_id, antonyms_game_tip_used=False
            )
            suggests = ["Подсказка"]
            return alice_request.response(
                d(
                    ANTONYMS_RIGHT_ANSWER + ANTONYM_NEXT_WORD, next_question=question
                ).text,
                buttons=suggests,
            )
        ...
        return alice_request.response(ANTONYM_WRONG_ANSWER, buttons=suggests)
    ...
    async def dont_know_handle(self, alice_request):
        ...
        if next_question is None:
            text = d(
                ANTONYM_DONT_KNOW + ANTONYMS_GAME_END,
                question=current_question[0],
                answer=current_question[1],
            )
            suggests = ["Хочу отгадывать антонимы!", "Главное меню"]
            return alice_request.response(text.text, buttons=suggests, tts=text.tts)

        suggests = ["Подсказка"]
        await self.dispatcher.storage.update_data(user_id, antonyms_game_tip_used=False)
        return alice_request.response(
            d(
                ANTONYM_DONT_KNOW + ANTONYM_NEXT_WORD,
                question=current_question[0],
                answer=current_question[1],
    ...
    def register_handlers(self):
        ...
        self.dispatcher.register_request_handler(
            self.game_process_handle, state=AntonymsGameStates.ANTONYMS_GAME
        )
